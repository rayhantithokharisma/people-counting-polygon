...
def detect(opt, *args):
    out, source, weights, view_img, save_txt, imgsz, save_img, sort_max_age, sort_min_hits, sort_iou_thresh, polygon_points = \
        opt.output, opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.save_img, opt.sort_max_age,\
         opt.sort_min_hits, opt.sort_iou_thresh, opt.polygon
    ...
    if webcam:
        ...
        dataset = LoadImages(source, img_size=imgsz, max_duration=150)
    ...
    for frame_idx, (path, img, im0s, vid_cap) in tqdm(enumerate(dataset), total = 3750): #for every frame
        ...
        for i, det in enumerate(pred): #for each detection in this frame
            ...
            if len(tracked_dets) != 0:
                for j, tracked_dets in enumerate(tracked_dets):
                    ...
                    frames_dict[identity].append(frame_idx)
                    
                    centroid_x = (bbox_x1 + bbox_x2) / 2
                    centroid_y = bbox_y2

                    if category == 0:
                        centroid_dict[identity].append((centroid_x, centroid_y))
                        if is_inside_polygon(centroid_x, centroid_y, polygon_points):
                            if identity not in passed_people:
                                passed_people.add(identity)
                                vehicle_count = vehicle_count + 1

            # Stream image results(opencv)
            if view_img:
                imx = im0
                w, h = imx.shape[1],imx.shape[0]
                cv2.polylines(imx, [polygon_points], isClosed=True, color=(255, 0, 0), thickness=2)
                #cv2.line(imx,(0,line_y),(250,line_y),(0,0,255),thickness=1)
                thickness = 3
                font = cv2.FONT_HERSHEY_SIMPLEX
                fontScale = 1
                cv2.putText(imx, "People Passing:  "+str(vehicle_count), (0, 30), font, 
    ...
    # frame_df = pd.DataFrame(list(frames_dict.items()), columns=['identity', 'frame_list'])
    # centroid_df = pd.DataFrame(list(centroid_dict.items()), columns=['identity', 'centroid_list'])

    # frame_df.to_parquet('frame_df.parquet', index = False)
    # centroid_df.to_parquet('centroid_df.parquet', index = False)

    if save_txt or save_img:
        if platform == 'darwin':  # MacOS
            os.system('open ' + save_path)

    return vehicle_count, len(frames_dict)
